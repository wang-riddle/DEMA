...
for (t in 1:TIMES) {
  ...
  SIGMA_hete=mvrnorm(1,rep(0,p),0.3^2*SIGMA)
  SIGMA_hete=crossprod(t(SIGMA_hete))+SIGMA
  Xs.list=list()
  Ys.list=list()
  for (k in 1:K) {
    Xs.list[[k]]=as.matrix(mvrnorm(ns,rep(0,p),SIGMA_hete))
...
write.csv(MSE.EST,"est2_1.csv")
write.csv(MSE.EST.SD,"estsd2_1.csv")
write.csv(MSE.PRE,"pre2_1.CSV")
write.csv(MSE.PRE.SD,"presd2_1.csv")
write.csv(WEIGHT,"weight2_1.csv")
